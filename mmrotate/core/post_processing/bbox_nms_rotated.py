...
def multiclass_nms_rotated(multi_bboxes: torch.Tensor,
                           multi_scores,
                           score_thr,
                           nms,
                           max_num=-1,
    ...
    inds = valid_mask.nonzero(as_tuple=False).squeeze(1).to(bboxes.device)

    bboxes, scores, labels = bboxes[inds.to(bboxes.device)], scores[inds.to(scores.device)], labels[inds.to(labels.device)]
    ...
    nms_clockwise = nms.get('clockwise', True)
    _, keep = nms_rotated(
        bboxes_for_nms, scores, nms.iou_thr, clockwise=nms_clockwise)

    if max_num > 0:
        keep = keep[:max_num]

    bboxes = bboxes[keep.to(bboxes.device)]
...
def aug_multiclass_nms_rotated(merged_bboxes, merged_labels, score_thr, nms,
    ...
    for cls in range(classes):
        ...
        nms_clockwise = nms.get('clockwise', True)
        cls_dets, _ = nms_rotated(
            cur_bboxes[:, :5],
            cur_bboxes[:, -1],
            nms.iou_thr,
            clockwise=nms_clockwise)
